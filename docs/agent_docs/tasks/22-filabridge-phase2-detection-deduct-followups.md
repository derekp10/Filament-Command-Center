...
> **Status: TODO** — filed 2026-06-13 by `/refresh-groups`. Three field-observed gaps in the new FCC-native print monitor, all surfaced by running the cancelled-print feature + dark FINISHED-completion path (`c1b0f5e`) live on the real fleet. **NOT compute bugs** — the deduct itself worked end-to-end on real XL multi-tool bgcode; these are detection-coverage + attribution + UX gaps. Part of the FilaBridge absorption epic (see the epic row in `working-groups.md`); knock these out alongside the Phase-2 cutover.
...
### 22.1 — Cancel detection: no instant ACK + fast cancel→restart slips the 30s poll (buglist line 3)
...
### 22.2 — Completion silently missed when the printer power-cycles before FCC observes FINISHED (buglist line 5)
