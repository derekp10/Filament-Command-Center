...
**Follow-ups (post-first-ship):** persistent badge/tray for pending reviews (today the entry point is the activity-log button + the `/api/cancel_deduct/pending` list — robust but discoverable only from the log line); FINISHED-completion deducts remain FilaBridge's until the Phase-2 atomic cutover; rung-2 `estimate × progress` fallback for un-prefix-parseable gcode (`.bgcode`) is still deferred (today such a cancel surfaces a "weigh the spool" warning rather than a low-confidence figure).
