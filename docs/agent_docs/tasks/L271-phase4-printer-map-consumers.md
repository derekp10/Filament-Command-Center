...
**Status (2026-06-04, end of session):** **Steps 1–3 DONE + pushed** on `feature/l271-phase-4` (origin, clean tree). Phase 3.5 is **DEPLOYED to prod** (`origin/main` `a070cff`). All `printer_map` READ consumers now source from the rows (dual-read); the editor GET reads rows too. **Everything so far is fully reversible** (config still on disk + synced). **Next = Step 4, the cutover** — start it in a FRESH chat.

> **▶▶ STEP 4 — START HERE (next chat).** Kickoff prompt: *"continue L271 Phase 4 — Step 4 (the printer_map cutover). Read `docs/agent_docs/tasks/L271-phase4-printer-map-consumers.md` (top), confirm you're on branch `feature/l271-phase-4`, then propose the Step-4 plan before editing."* **Branch:** `feature/l271-phase-4` (the dev container bind-mounts the working tree, so it must be checked out). **First decision to put to Derek:** when retiring `config:printer_map`, DELETE it from config.json (clean, but a prod data change + the EBUSY single-file bind-mount gotcha [[reference_fcc_config_json_bind_mount]]) vs LEAVE it vestigial (safer, delete in a later step) — agent recommends **leave vestigial**. **Step 4 work:** (1) rework `/api/printer_map` PUT (`app.py:~2801`) to write `toolheads[]` onto the Printer rows (group editor payload by printer prefix), creating a Printer row for a brand-new printer; (2) point `_printer_map_blocked_removals` (`app.py:~2710`) at the rows (old_map = `get_active_printer_map()`); the guard logic (scan slot_targets + Spoolman occupancy) stays; (3) drop the config fallback in `locations_db.get_active_printer_map` so rows are sole source; (4) retire `config_loader.save_printer_map` / `_canonicalize_printer_map` / the `load_config` key-uppercasing (or repurpose for the row write-path) + the startup fold migration becomes a no-op. **Test seams** (when a swap reddens a test): stub `locations_db.get_active_printer_map`, or an autouse fixture delegating it to the stubbed `config_loader.load_config` (see `test_active_print_backend_enforcement.py`, `test_filabridge_recovery.py`). Keep the `/api/printer_map` GET/PUT shape so the 4 JS modules stay unchanged.
...
```
**How it was configured (NOT FilaBridge):** you typed it into the FCC **Config/Settings modal → the "Toolheads" / Printer-Map editor** (`inv_settings.js` `renderPrinterMap`/`pmRender` → GET/PUT `/api/printer_map` → `config_loader.save_printer_map`). Each row = a toolhead's LocationID + which printer it belongs to (the `printer_name`, emoji and all) + its slot index (`position`). The **printer names you typed there are literally what defines the printers** — "🦝 XL" and "🦦 Core One Upgraded" exist because you entered them here. FilaBridge is separate: it supplies printer *credentials/state* keyed BY `printer_name` (so FCC can query PrusaLink), but it never populated this map. **`position`** is the toolhead's MMU/slot index; it's load-bearing for the MMU M0/M1 de-dup (two aliases can share a position) and for the dryer-box slot ordering. **What L271 Phase 4 is doing:** moving this map OUT of `config.json` and ONTO each first-class `Type:"Printer"` row in `locations.json` as a `toolheads:[{location_id, position}]` array (the `printer_name` becomes the Printer row's `Name`). Steps 1–3 made everything READ from the rows; Step 4 makes the editor WRITE to the rows and retires the `config.json` copy.
