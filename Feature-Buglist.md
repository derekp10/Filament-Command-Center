...
* Loading a dryer box, at the very least, slots load with an initial data set, and then i think the 5 second tick goes off, and fills them in completely. Seems to be a difference between what the initial card load does, and the data provided from the 5 second tick. We should probably make the initial card load load the same data as the 5 second tick.
    - **Investigation 2026-04-22 (deferred)**: two consecutive `/api/get_contents?id=<db>` responses 6 s apart are byte-identical; automated Playwright probe of `CR-MDB-1` shows the initial open renders all 4 cards complete on first paint, the 5-s sync tick's hash guard (`state.lastLocRenderHash`) short-circuits, and closing/reopening produces byte-identical card HTML. The perceived flicker is not reproducible in headless Chromium. Likely real cause on a live browser: `openManage` wipes `#slot-grid-container.innerHTML = ''` BEFORE the fetch resolves ([inv_loc_mgr.js:82](inventory-hub/static/js/modules/inv_loc_mgr.js#L82)), so the user briefly sees an empty grid for ~100-300 ms while the fetch is in flight, and QR codes paint in `requestAnimationFrame` after cards do. Suggested follow-up: either (a) delay the wipe until the fetch resolves, or (b) render skeleton slot placeholders during the in-flight window.
