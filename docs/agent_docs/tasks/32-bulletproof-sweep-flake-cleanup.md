...
> **Status: ✅ DONE 2026-07-05** (`feature/group-32-bulletproof-flake-cleanup` → `dev`+`main`). All 3 targeted flakes fixed + verified GREEN in the full `RUN_INTEGRATION=1` sweep. **32.1** root cause pinned: the `data/*.json` stores are bind-mounted into the container, so the host pytest and the in-container cancel-monitor daemon `os.replace` the same target simultaneously → Windows `PermissionError`. Fix = new shared `atomic_store.replace_with_retry` (bounded retry on Windows PermissionError, no-op on POSIX), routed through **all four** same-pattern stores (`cancel_fetch_store`, `cancel_review_store`, `print_deduct_ledger`, `print_tracker_store`); pinned by `test_atomic_store_retry.py` (5 cases). The `encoding='utf-8'` half was already present (buglist misdiagnosis). **32.2** = a transport-only bounded-retry `_req` helper (15s floor, re-raises after N — no masking) routed through the two flaky `bulk_set` tests + shared request helpers. **Sweep result: 2129 passed / 5 failed / 14 skipped — the 3 targets GREEN; the 5 residual failures A/B-proven pre-existing (fail identically with this diff stashed), NONE from Group 32, filed as a fresh Testing item (Group-33 candidate: 3 buffer-poll load flakes + a stale quickswap visual baseline (overlay 284→502px) + a buffer-population E2E).** Full write-up in `completed-archive.md`.
>
