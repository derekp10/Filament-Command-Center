...
> **Status: ✅ DONE 2026-07-05** (`feature/group-27-l316-charact-reds` → `dev`). All 10 🔴 reds fixed, each with its `tests/test_l316_charact_*.py` pin flipped to assert the corrected behavior in the same change (2 pins renamed; 27.10 gained a new filament-side pin). Two UX forks decided with Derek — 27.5 = no-op + info line (preserves audit state), 27.6 = keep slotted spools + surface survivors. 562 backend tests pass, 0 regressions. Detail in `completed-archive.md`. _(Originally filed 2026-07-01 by `/refresh-groups`: the 10 🔴 "real bug, fix soon" findings from the **L316 characterization layer**. Every finding was PINNED as *current* behavior by a named test in `tests/test_l316_charact_*.py`. Full annotated write-up + pin-test names: [L316-characterization-findings.md](L316-characterization-findings.md), numbered 1–50, matching the `(N)` tags below.)_
