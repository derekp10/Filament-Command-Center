...
| 2a | **Cancel DETECTION + latching** — `_PRINT_TRACKER` + `_track_print_edge` in `fetch_for_printer`: latch `filename`/`job_id`/monotonic `progress`/`file_meta` while PRINTING via a **`/api/v1/job` fetch** (`prusalink_api.get_printer_job`; `get_printer_state` lacks them); on the →STOPPED/ERROR edge fire the cancel action **async** (daemon thread, doesn't block the heartbeat). STOPPED/ERROR-only; offline/empty-state polls preserve the latch (bridge transient blips). Hardened from a 12-finding adversarial review: `.bgcode` rejected up front (binary body un-prefix-parseable), a cancel that can't be computed surfaces a **visible** WARNING (never silently un-deducted), blank/zero job_ids handled. | M | ✅ `14acbcd` (37 cancel-suite tests) |
| 5 | **Preview-and-confirm UX** — the cancel edge now computes the per-tool partial and STASHES it (`cancel_review_store.py`, persistent + restart/scroll-safe) instead of auto-writing; a `🛑 Review` activity-log button opens a `mountOverlay` preview (`cancel_review.js`) with nudgeable per-spool grams + live remaining; `/api/cancel_deduct/{pending,confirm,dismiss}` apply it (confirm re-reads CURRENT used → additive, atomic `pop_pending` claim, sends only `{used_weight}`). `deduct_cancelled_print` kept as the compute-and-apply primitive (shares `_compute_cancel_usage`). | M | ✅ this branch (backend + 3 Playwright tests) |

**Follow-ups (post-first-ship):** persistent badge/tray for pending reviews (today the entry point is the activity-log button + the `/api/cancel_deduct/pending` list — robust but discoverable only from the log line); FINISHED-completion deducts remain FilaBridge's until the Phase-2 atomic cutover.

### 9.9 BINARY-GCODE REALITY + decoder + monitor (2026-06-11) — `84e7c13`

Live probing of the real fleet (XL + Core One, via the FilaBridge credential path) changed two load-bearing assumptions:

1. **The whole fleet slices to BINARY gcode (`.bgcode`)** (424/424 files on the Core One; the XL's live print too), and **PrusaLink exposes NO measured filament metadata over the LAN** — `/api/v1/job` and the files endpoint both return empty `meta`. The printing file is **fully 404 until the print STOPS** (confirms download-at-cancel-time). So rung-1's prefix-parse — and even rung-2's footer estimate — needed the file BODY, which is heatshrink+MeatPack-compressed inside the bgcode container. **The shipped prefix-parse read zero of Derek's real prints.** Derek chose to **add a bgcode decoder** (over switching the slicer to ASCII or using Connect).
   - **`bgcode_decode.py` (NEW, pure-Python, zero deps):** block-walk + `zlib`(deflate) + hand-rolled **heatshrink(11/4, 12/4)** + **MeatPack**. heatshrink validated **byte-equal to the reference `heatshrink2` lib** on every real block; a real 231KB file decodes + prefix-parses to the slicer footer (**tool 1 = 6.43 g vs 6.35 g, 1.3 %**), validated end-to-end on the live printer. It lifts the per-tool `filament used [g]/[mm]` footers out of the (deflate) **PrintMeta** block (they're NOT in the bgcode body) and maps the printer's `progress` (a byte position in the COMPRESSED file, behind the header/thumbnails) to the decoded text. Hardened (49-agent review): per-block try/except + bounds + 64 MB/block bomb cap. So **rung 1 (exact per-tool) now works on the binary fleet — rung 2 is no longer needed for Derek.** `prusalink_api.fetch_cancel_gcode` does download+decode+remap; `parse_partial_filament_usage` gained no-spaces tolerance, **high-water-mark** E accounting (retraction re-primes no longer double-count: 7%→1.3%), bare-G92 reset, and a single-material-MMU no-`Tn` tool fold.

2. **Detection must be dashboard-INDEPENDENT (Derek's correction).** It rode `_pulse_section_printer_status`, which only runs when a browser has the dashboard open + at a focus/visibility-dependent cadence — so unattended cancels (the norm) were missed. Moved to a dedicated **`_cancel_monitor` daemon** (30 s fixed tick, fan-out probe, idempotent, started from `__main__` only). `_track_print_edge` removed from the pulse (widget-display only now).

**Still open for the cancelled-print feature: Derek's physical sign-off only** — a real cancel to confirm the live `/api/v1/job` shape (already matched the probe) + the per-tool→toolhead-position mapping on his actual printer_map (the XL toolchanger maps tool index→position 1:1; the Core One MMU may map several tool indices→one physical head — `_apply_usage_to_printer` now logs a visible warning if a tool index matches no position). **Known caveat:** FilaBridge's own error-recovery path (`download_gcode_and_parse_usage`) still can't read bgcode (pre-existing; left alone to preserve its Range fast-path).

### 9.10 Real-data XL multi-tool decode validation (2026-06-12) — ✅ PASS

Pulled a real **2-tool** `.bgcode` off the **live XL** (`usb/BINS-X~5.BGC` — "Bins - XL PETG,PETG", 240 KB) via the FilaBridge cred path, single-connection streamed download with a wall-clock TOTAL timeout, stream fully consumed + closed. This closes the only real-data gap that remained after §9.9 (which had only validated the binary decoder on the Core One's *single-material* files + synthetic XL maps).

- **Download / link:** `http://{ip}/usb/{file}` → 200 / magic `GCDE`. The XL **ignores `Range`** (`Content-Range: None` → always full-file). Measured throughput ≈ **14 KB/s** (Buddy's tiny pool; a 10 MB file ⇒ ~12 min). NB: the bare `http://{ip}/{file}` (no `usb/` prefix) 404s — that earlier looked like the §9.10 selected-file *lock* but was just the wrong path; the real download path serves 200 even for the selected/FINISHED file.
- **Decode:** `is_bgcode` ✓; 240 KB → 415 KB gcode; 7 gmap blocks; **123 M73 markers 0→100 %**. Footer lifted from the PrintMeta block = a genuine **5-position multi-tool array** `0.00, 4.48, 0.00, 0.00, 12.51` — the slicer used the **non-adjacent** extruders **1 and 4** (MeatPack decoded both `Tn` runs correctly).
- **(a) per-tool grams, both engaged heads:** `parse_partial_filament_usage(gcode, 1.0) = {1: 4.40, 4: 12.38}` ≈ footer `[_, 4.48, _, _, 12.51]` within ~1–2 % (the high-water-mark E parse omits the final wipe/ram the footer estimate keeps); untouched tools 0/2/3 absent = 0.
- **(b) untouched-head = 0 at a partial fraction:** at progress `0.01` → `{4: 0.119}` only — **tool 1 absent** (the XL invariant, on real data).
- **(c) M73 mapping sane:** `progress_to_decoded_fraction` monotonic + bounded, with the documented non-linear time-vs-byte shape (`0.50→0.30`, `0.90→0.84`), each tool climbing to the footer at `1.0`.
- **(d) per-position spool resolution:** prod `_resolve_usage_to_spools('🦝 XL', {1:.., 4:..})` → tool 4 → pos 4 → **XL-5 / #230** (12.38 g); tool 1 → pos 1 → XL-2 which is **EMPTY in dev**, so correctly **no row**. Synthetic maps over populated positions (`{0,4}` → XL-1/#86 + XL-5/#230; `{0,3,4}` → 3 distinct rows) prove each tool index routes independently to its own position (XL printer_map pos 0..4 = XL-1..XL-5, clean 1:1). Current dev XL layout: pos0 XL-1=#86, pos1/pos2 EMPTY, pos3 XL-4=#230, pos4 XL-5=#230.

Read-only throughout (no Spoolman write); probe scripts deleted, no source change. The decoder is format-level, so the literal last print `MICROM~1.BGC` (PLA,PLA, 10.4 MB) would exercise the identical path — left un-pulled as redundant (~12 min download). The `_cancel_monitor` daemon was confirmed alive and live-catching Core One cancels across all three paths (immediate / deferred-fetch-on-lock / confirm-write) in the container logs. **Only a physical XL cancel (live-flow mirror of the Core One) remains — a flow check, not a logic gap.**
