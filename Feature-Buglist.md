...
* ~~MMU mode weight deduction: M0 and M1 map to the **same filabridge slot** (`CORE1-M0/1`). M0 = direct-feed port on the side of the printer (no MMU). M1 = filament routed through the MMU. By design they are mutually exclusive during a print — both should never deduct simultaneously. **Decision: Prefer (a) — query PrusaLink/firmware for the active MMU-enabled flag per print via the `/api/printer_state/<id>` endpoint** to know which mode is active and deduct only from the correct slot. If the MMU flag can't be reliably derived, fall back to (b) — treat `M0` and `M1` as aliases of the same slot and deduct only once (whichever filabridge reports as active). The fix lives in the filabridge usage-map consumption layer in `logic.py`.~~ _[DONE 2026-04-23 — the fix lives in `app.py` (not `logic.py`, which contains the move/fb-write plumbing; usage deduction is in the `/api/filabridge_error` handler and its auto-recover sibling). **PrusaLink probe findings:** `/api/v1/info` exposes an `mmu: <bool>` field — it's hardware-static (reflects MMU attachment, not per-print routing) but usable as the "preferred alias" signal. `/api/v1/status` does NOT expose MMU state. `/api/v1/job` and `/api/printer` also don't. **Implementation:** new `prusalink_api.get_printer_mmu_flag()` helper + `app._resolve_active_locs_for_printer()` which re-orders printer_map candidates so the preferred alias (`-M1` when mmu=True, `-M0` when mmu=False, insertion order when unknown) is iterated first. Both deduction loops then track processed positions and only deduct from the FIRST alias per position that has a spool — guaranteeing each gcode usage slice is applied exactly once. Tests in `test_mmu_alias_dedup.py`.]_
...
* ~~Possible issues with >1kg spools and tracking weights?~~ _[DONE 2026-04-23 — regression coverage lives in `test_large_spool_weight_tracking.py`. Five cases: 3kg initial_weight stored intact, 1500g partial deduction persists verbatim (no silent 1kg cap), 2999g deduction persists, over-deduction clamps at initial_weight (existing `update_spool` cap holds), successive deductions accumulate correctly. All green on the dev Spoolman.]_
