...
* **[CRITICAL · PRIORITY]** Real Spoolman integration tests (`pytest.mark.integration`). Today every test mocks `/api/filaments`, `/api/update_filament`, `/api/update_spool` — mocks always echo back what we sent, so server-side behaviors are completely invisible to the suite. This gap directly let through the 2026-04-26 PATCH-replaces-extras data-loss bug (filament 157 lost `product_url`/`purchase_url`/`original_color`) AND the 2026-04-27 prod-breaking sanitize bug (numeric-string text extras like `prusament_length_m: "330"` got unwrapped → Spoolman 400 → slot assignments stuck, label-confirm notifications missing, force-moves no-op). **Both classes of bug would have failed loudly in CI with a single integration test that creates a real test filament/spool, exercises the full update chain, reads the record back, and asserts the field survived correctly.** Plan: per-test fixture creates + deletes a fresh test record (dev Spoolman is isolated from prod, pollution risk is low), `pytest.mark.integration` so host pytest stays fast on default runs, hard-skip when the dev Spoolman host (192.168.1.29:7913) is unreachable. Pin specific scenarios: partial-PATCH preserves siblings, text-type numeric-string extras survive round-trip, label-scan flips `needs_label_print` without wiping other extras, slot-assignment scan succeeds for spools carrying prusament_* extras.

* **[CRITICAL · PRIORITY]** Silent-fail audit on every `update_spool` / `update_filament` caller. Both functions return `None` on Spoolman rejection and most call sites either log a generic "failed to update" or don't check the return value at all. The actual Spoolman error body lives in `spoolman_api.LAST_SPOOLMAN_ERROR` (already populated by `update_filament`; should also be by `update_spool`). User-facing impact today: when a partial PATCH gets rejected, the UI shows "nothing happened" — no toast, no activity-log entry, just dead silence. The 2026-04-27 prod outage stayed undiagnosed for hours because every visible symptom (slot stays unassigned, label not confirmed, force-move no-op) gave the user no signal about WHY. Plan: (1) every caller that does `if update_xxx(...)` adds an else branch that surfaces `LAST_SPOOLMAN_ERROR` in the toast/activity-log, (2) callers that ignore the return value get audited and either fixed or annotated with `# fire-and-forget — explain why`, (3) consider an exception-raising variant (`update_filament_or_raise`) for paths that genuinely should never silent-fail (slot assignment, label confirm, force-move). Pairs naturally with the integration tests above — failures should be visible AND tested.

* **[CRITICAL · PRIORITY]** Spool label-confirmation scan still not firing the "✔️ Spool #N Label Verified" notification. User reports as of 2026-04-27 evening that scanning a spool barcode (`ID:N` format) on a record with `needs_label_print: true` does NOT produce the success activity-log entry, even after the JSON_STRING_FIELDS sanitize fix shipped earlier in the day. Code path is `app.py:1314-1328` → `spoolman_api.update_spool(sid, {'extra': extra})`. Possibilities to check in order: (1) is the running container actually on the latest image (`docker compose pull && up -d` on prod) — if the wizard's filament-side label-confirm at `app.py:1360-1378` works but the spool-side at `app.py:1314-1328` doesn't, the fix is live and there's a separate spool-only bug; (2) is `update_spool` returning None silently due to some other extra field rejecting (logs would show the 400 reason — see the silent-fail audit entry above); (3) is the dirty-flag check itself failing (e.g. `needs_label_print` arriving as a different truthy form than the three the code checks for: `True`, `'true'`, `'True'`); (4) is `parse_inbound_data` unwrapping `needs_label_print` to a boolean such that none of the three string forms match anymore. Diagnostic first step: scan a spool with DevTools Network tab open, look at the `/api/identify_scan` response AND the docker container logs for any Spoolman 400 around the same timestamp. Until this is fixed, blind-scanning workflows can't trust that labels were marked confirmed — the user has no signal either way.

* **[CRITICAL · PRIORITY]** Editing a spool's filament data while slotted into a print head causes it to be removed from its toolhead assignment — or the location shows correct but registers as empty (0/1) in the location list modal with nothing assigned in Location Manager. Same extras-clobber class as the integration-test and silent-fail audit items. _[COLLABORATIVE REPRO NEEDED — wizard save likely writes `extra` without read-merge-write, wiping `container_slot`. Steps when ready: (1) Spoolman DB dump before editing, (2) edit a spool actively slotted into a toolhead via the wizard, (3) save, (4) DB dump after, (5) capture Network tab POST payload. Candidates: wizard save clobbering `container_slot`, filabridge unmap triggered by a stray update, or `extra` field overwrite missing read-merge-write pattern.]_

* **[CRITICAL · PRIORITY]** Spools not actively deployed to a toolhead lose their `container_slot` assignment when a new version is deployed to production. _[CONFIRMED 2026-04-26 — Reproducible. Trigger: deploying a new version/code update to production. A plain Docker container restart does NOT appear to cause it (not observed on dev). Culprit is something in the deployment or migration path that runs only when new code is loaded, not generic startup/init. Same extras-clobber class. Suspected cause: `container_slot` cleared when `update_spool` runs without merging prior extras. Fixed in `perform_smart_move` (read-merge-write at logic.py:321-330); other surfaces may still write bare `{'extra': {'container_slot': ''}}` and clobber siblings. Next step: audit all `update_spool` callers + deployment-time code paths (migrations, setup_fields.py, any one-shot init that touches spool extras).]_

* **[CRITICAL · PRIORITY]** Failed to verify Filament #126 label after QR code scan of newly printed label. Same silent-failure class as the label-confirmation bug — scan fires but label status doesn't update, user gets no signal why. Likely a silent `update_spool`/`update_filament` return-value being ignored, or a dirty-flag check mismatch. The silent-fail audit should catch the exact caller responsible.

* **[CRITICAL · PRIORITY]** Review and unify update logic across the program — too many orphaned/divergent update paths causing production data-loss. Every extras-clobber and silent-fail bug traces back to multiple independent write surfaces that don't share a read-merge-write pattern. _[Promoted from ON HOLD — the 2026-04-26/27 prod incidents (PATCH-replaces-extras, sanitize bug, slot-assignment loss, label-confirm silent failure) proved this is the architectural root cause, not cleanup. Plan: inventory every caller that writes to Spoolman spool/filament/location records, classify by "edit surface" (wizard, quick edit, scan handler, auto-unmap), define a single dirty-diff helper that every surface funnels through, migrate one surface at a time. Pairs with the silent-fail audit.]_

* **[CRITICAL · PRIORITY]** Address outstanding issues in `setup_fields.py` — non-destructive handling of existing choice fields + error code checking. Runs on deployment and touches spool/filament extras; if it writes extras without read-merge-write, it's a prime suspect for the confirmed deployment-triggered slot-loss bug. _[Promoted to CRITICAL — directly tied to the deployment-path extras-clobber investigation. Needs audit: does `setup_fields.py` PATCH extra field definitions in a way that could trigger Spoolman to reset/clear existing extra values on spools? If so, that's the smoking gun for the "spools lose container_slot on version update" bug.]_
