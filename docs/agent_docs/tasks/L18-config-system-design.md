...
Resolved after review with Derek: **(E)** kept export/import exactly as-is (verified working live — redacted export → import preserves the key), and added a defensive read-side guard so `load_config()` neutralizes the `__secret_set__` sentinel if a redacted export is ever *copied into place* instead of imported (treats it as unset rather than serving the placeholder as the real key). **(F)** parent-directory mount **DECLINED** — the code fixes above make the single-file mount fully safe; the durable `.bak` now lands on the host-visible `inventory-hub/data/` (reachable via Derek's SMB share to the app dir); and keeping the single-file mount avoids dev/prod drift and leaves the `update.sh` cron deploy unchanged (`git reset --hard origin/main` picks up the code with no rebuild since the code is bind-mounted `./:/app`; `config.json` + `data/*.bak` are gitignored so the reset never clobbers them).
