...
>
> **FOLLOW-UP 2026-06-04 (Derek semantic cleanup + "add new types"; commit `e7ccea3`).** **Renamed the shelf levels to Room → Wall Shelf → Row → Section** (Derek: "Walls should be Wall Shelfs … SC are sections on a row"). `WALL_TYPE` `'Wall'`→`'Wall Shelf'`; new `SECTION_TYPE` `'Section'` is the **spool-holding leaf** (Wall Shelf + Row stay structural, excluded from pickers; Section is offered). The migration now also **retypes** legacy `'Wall'`→`'Wall Shelf'` and the `'Shelf'` section leaves→`'Section'` (alias-aware so the dev-transition state folds idempotently; only pattern-matching ROOM-WALL-ROW-SECTION leaves). Badges reordered (exact `Wall Shelf`/`Row`/`Section` before the generic `Shelf` includes, since "Wall Shelf" contains "Shelf"). **Custom Types:** `#edit-type` is no longer a hardcoded list — it's JS-populated (`_populateTypeSelect`) from `_LOC_BUILTIN_TYPES` + custom types (localStorage `fcc.locMgr.customTypes` + any Type already used on a row), plus a free-text `#edit-type-custom` input to add a brand-new type on the fly. Editing a custom-typed row no longer drops its Type. **Prod note updated:** the deploy now creates Wall Shelf/Row rows AND retypes the 8 prod `Shelf` sections → `Section` (idempotent). Live-verified on dev (badges Wall Shelf/Row/Section, Type dropdown + custom input). 26 unit + 5 integration + 303 broad regression green.
