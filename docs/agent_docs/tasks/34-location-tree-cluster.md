...
> - ⚠️ **OPEN REFINEMENT — auto-gen id (filed 2026-08-02, Derek):** the shipped S1–S4 auto-gen-id is finicky. (1) When you ➕ on a base location then change its parent/type in the Location section, the auto-id doesn't re-sync to the new breadcrumb (you get `CR-CT1-R1` where it should follow the new `CR-XXN` id). (2) Numbering isn't topology-aware — it always proposes `R1` and would overwrite an existing sibling instead of incrementing; if an `R1` already exists under that parent, the next row should auto-populate `R2`. Derek: "make it either easier, or clearer to the user." Refines the add-redesign auto-id inference (`inv_loc_mgr.js` add-child) — this is a follow-up on the SHIPPED S1–S4, distinct from the deferred S5. Address before the add-redesign is called fully done.
> - ✅ **Bulk Moves (L298) — COMPLETE, ALL 5 PHASES MERGED to `dev` 2026-08-03.** Phase 0 undo-hardening (`b4a26e8`) + Phase 1 `POST /api/bulk_move` (`96f87bc`, undo hotfix `f45afe9`) + Phase 2 `CMD:BULKMOVE` session/deck-slot/LM "Move all →" (`4b0e0fe`+`7fa990b`+`b56149f`) + **Phase 3 rich preview/confirm panel** (`d706d43` — folded in all 17 deferred Phase-2 findings + 14 from a fresh 8-agent review) + **Phase 4 `Shift+B` entry + edge hardening** (`1d8e3f7`+`192eb96` — 23 findings) + the **armed-pill affordance fix** (`43b3801`). Merged `7833550` + `d11f156`. Full as-built in [L298-bulk-moves-plan.md](L298-bulk-moves-plan.md).
>
> **📌 Status decision 2026-08-06 (`/refresh-groups`, Derek): this group STAYS `PARTIAL`** and keeps its two open slices (S5 + the auto-gen-id refinement) rather than lifting them into the new redesign group. **🔗 Sequence both AFTER [Group 37 — 🌳 Location System Redesign](37-location-system-redesign.md)**, which is scoped on a proven root cause (`location_prefix` splits on the FIRST dash, so a room reaches its whole subtree while every other level is exact-match-only) and has **4 open forks — including whether the human-readable composite LocationID is still the right model at all.** Both leftovers here build directly on semantics that fork would change: S5 creates missing levels, and the auto-gen-id proposes composite ids. Building them first risks doing the work twice.
