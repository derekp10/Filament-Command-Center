...
* Filament Edit follow-ups (Edit Filament MVP landed 2026-04-22):
  ...
  * **Edit Filament Wave 2 (NEXT)** — remaining gaps in the direct Edit Filament form vs. the full wizard:
    1. Searchable Material combobox (today it's a plain text input; wizard uses a typeahead over the existing-materials list).
    2. Searchable Vendor combobox with "create new vendor" inline (today it's a flat `<select>`; wizard uses `wizardBindCombobox` + a hidden `wiz-fil-vendor-new` input for new vendors).
    3. Dynamic `extra` fields editor — mirror wizard's `wiz-fil-dynamic-extra-fields` so we can edit `filament_attributes` (multi-select like "Silk"/"Carbon Fiber"), `product_url`, `purchase_url`, `sheet_link`, `original_color`, `sample_printed`, `flush_multiplier`, etc.
    4. External-metadata import panel (wizard's `wiz-fil-external-metadata` — pull from Prusament product pages / open-filament-database).
    _[NEEDS DESIGN — the Swal-based form is getting crowded. Next iteration should probably promote Edit Filament from a SweetAlert to a proper Bootstrap modal so we can use tabs/sections and reuse more wizard code directly. The alternative — keep Swal and add a "Show more fields" collapsible — would extend the MVP but won't scale to the full wizard feature set.]_
