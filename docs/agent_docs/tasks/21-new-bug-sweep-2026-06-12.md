...
> **Status: DONE 2026-06-15** — all 6 shipped on `feature/new-bug-sweep-2026-06-12` → merged to `dev`. Each was Playwright-reproduced/verified against the live dev container. 17 new/rewritten tests; 122 passed across adjacent suites, 0 regressions. Resolutions (anchor by quoted text, not the stale line numbers):
> - **21.1** — the "lower-right search badge" = the floating `.fcc-fab-search` (measured overlapping the bottom buffer-card weight). Per Derek (uses it more than the navbar button, must stay always-available incl. over modals, do NOT remove): reworked into a **drag-to-park FAB** — new `fab_drag.js` (Pointer-Events drag mouse+touch, position persisted to `localStorage:fcc.fab.pos`, viewport-clamped, default = bottom-left cmd-deck band clear of buffer weights + WEIGH QR, tap=open/drag=move/long-press=reset) + `/` and `Ctrl/Cmd+K` search shortcuts (none existed). `test_fab_no_overlap_with_cmd_deck.py` rewritten → 8 tests; visual baseline within tolerance.
> - **21.2** — self-healing QR sweep added to `_renderManagePayload`'s anti-wiggle early-return (`inv_loc_mgr.js`): re-fills only EMPTY `qr-pick/print/trash` placeholders from each card's `data-spool-id`, so a dropped rAF QR pass recovers within one dashboard pulse. (Could NOT deterministically reproduce the race in 10+ eject cycles — this is a low-risk safety net, not a move-path change.)
> - **21.3** — `perform_smart_move`'s `is_toolhead` single-occupancy check (`logic.py`) omitted `'No MMU Direct Load'`; now sourced from canonical `locations_db.TOOLHEAD_TYPES` (∪ `'Printer'`). 4 regression tests (`test_toolhead_resident_eject_21_3.py`). (The Location-Manager/Force-Location `manage_contents` add path already auto-ejected for `Tool Head` — verified live.)
> - **21.4** — dropped `modals.spoolModal.hide()` from the `btn-print-action` handler (`scripts.html`); toast confirms. Sibling of 17.2.
> - **21.5** — `dblclick` handler in `promptEditLocation` (`inv_details.js`) selects the row + `Swal.clickConfirm()` (reuses the commit path); single-click+Force and keyboard nav unchanged.
> - **21.6** — deterministically reproduced (stale `/api/state/buffer` pulse resurrecting a just-assigned spool past the 3s grace window). Added a "recently-assigned-out" guard in `inv_cmd.js`: `loadBuffer`'s overwrite filters out spools the client just assigned out + re-asserts the corrected buffer; re-buffering clears the mark. 3 tests. Full bidirectional multi-client sync remains L302 (ON HOLD).
> - **Spun off (standalone in buglist):** the `locations_db.TOOLHEAD_TYPES` double-definition code smell (set @20 + frozenset @547), surfaced while fixing 21.3.
