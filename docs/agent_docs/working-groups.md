...
| Confirm-change modal blocked during print (L122) | `PARTIAL` 2026-05-14 — scan-side `_confirmActivePrintScan` migrated to `mountOverlay`. Manage-modal slot-assign path (`_confirmActivePrintAssign` in `inv_loc_mgr.js`) still hand-rolled; needs same migration if symptom recurs on slot-click path. Also see new L361 (PRINTING-state test flakes — likely the same hand-rolled overlay intercepting test scenarios). |
...
| **NEW** Test flakes when a printer is in active PRINTING state (L361) | 6 tests in `test_printer_status_widget.py` + `test_contrast_guard.py` time out waiting for `#manageModal` to become visible after clicking a Printer-Status toolhead tile/chip — but only when Derek's Core One is `PRINTING`. Confirmed pre-existing (also fail on `dev` parent commit). Hypothesis: the L122-unmigrated `_confirmActivePrintAssign` slot-click overlay is intercepting. Fix-paths: (a) stub `is_active=false` printer in fixtures, or (b) finish the L122 migration and have the manage-modal overlay dismiss-on-click. |
