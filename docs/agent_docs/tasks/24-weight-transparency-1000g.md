...
> **Status: TODO** — filed 2026-06-15 by `/refresh-groups`, both items grounded against real code by a 7-agent triage workflow + a direct `1000` grep. Two items on the weight-write surface: **F** = manual weight changes leave no before→after trail; **K** = hard-coded 1000g spool logic that should be variable by size.
