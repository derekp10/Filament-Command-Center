...
> **Status: ✅ DONE** 2026-07-03 (`feature/group-26-test-infra-debt`, `8f1feab`). All 21 reds + both flakes cleared and 7 net-new FE tests added; a full run of the 15 touched files = **175 passed / 0 failed** (`RUN_INTEGRATION=1`, 4:47). Test-only, no product code. Per-item notes below (26.1–26.9). Two things worth knowing for the future: (a) the `bound_loaded_slot`/`real_toolhead` fixtures live in `conftest.py` and are the canonical way to get an overlay-ready toolhead; (b) `test_visual_quickswap_confirm_overlay` now snapshots the **bounded confirm card** (not the full-viewport backdrop) — its text is tied to whichever loaded slot the fixture picks (sorted-first, deterministic in isolation), so a full-sweep data drift is a `UPDATE_VISUAL_BASELINES=1` recapture, not a per-run flake.
>
> _Filed 2026-07-01 by `/refresh-groups` (consolidation). Bundled the **21 pre-existing reds** cataloged on `feature/group-24-weight-transparency-1000g` (clean full sweep: 21 failed / 1759 passed / 10 skipped, `RUN_INTEGRATION=1`, ~22 min; ALL 21 confirmed PRE-EXISTING, none touch product surfaces) **PLUS the intermittent flake rows + the Group-23 FE test-backfill** that the reds item itself named "siblings of the same test-infra debt." One batchable session so a green sweep becomes a real signal._
