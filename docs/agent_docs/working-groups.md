...
| Confirm-change modal blocked during print (L122) | `DONE` 2026-05-25 on `feature/buglist-sweep-2026-05-25`. Both code paths now go through `window.mountOverlay`: scan-side `_confirmActivePrintScan` migrated 2026-05-14, manage-modal slot-assign `_confirmActivePrintAssign` migrated 2026-05-25 with `tier='confirm'` + `host=manageModal` so the overlay rides above any Bootstrap modal and cleans up on host close. Mirrors the `_confirmActivePrintScan` reference impl exactly (Escape via `onEscape`, focus trap via keyHandler, QR-confirm pair via `attachConfirmQRs`). Same migration also defuses the L361 PRINTING-state flake risk surface — the hand-rolled overlay was suspected of intercepting toolhead-tile clicks. |
...
| Test flakes when a printer is in active PRINTING state (L361) | `DONE` 2026-05-25 on `feature/buglist-sweep-2026-05-25`. Fix path B (finish the L122 migration) chosen, plus a defensive companion: (1) `_confirmActivePrintAssign` migrated to `mountOverlay` so it can no longer intercept toolhead-tile clicks; (2) Printer Status widget click handler moved to event delegation on the stable widget root so clicks survive arbitrary body re-renders. Regression coverage in `test_printer_status_widget.py::test_l361_toolhead_click_survives_rerender` (forces 3 full re-renders then asserts the click still opens the manage modal). |
