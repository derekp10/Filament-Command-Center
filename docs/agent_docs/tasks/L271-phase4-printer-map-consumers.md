...
> **▶▶ STEP 4 — START HERE (next chat).** Kickoff prompt: *"continue L271 Phase 4 — Step 4 (the printer_map cutover). Read `docs/agent_docs/tasks/L271-phase4-printer-map-consumers.md` (top), confirm you're on branch `feature/l271-phase-4`, then propose the Step-4 plan before editing."* **Branch:** `feature/l271-phase-4` (the dev container bind-mounts the working tree, so it must be checked out). **First decision to put to Derek:** when retiring `config:printer_map`, DELETE it from config.json (clean, but a prod data change + the EBUSY single-file bind-mount gotcha [[reference_fcc_config_json_bind_mount]]) vs LEAVE it vestigial (safer, delete in a later step) — agent recommends **leave vestigial**. **🛑 HARD CONSTRAINT (verified 2026-06-04):** `position` IS the FilaBridge `toolhead_id`, sent verbatim. The Step-4 PUT must **preserve `position` exactly — never renumber/compact/normalize** (the XL-5→5 gap at 4 is a real FB hardware index). Add a test pinning gap preservation through a PUT round-trip. `printer_name` stays ONLY on the Printer row's `Name` (do NOT store it per toolhead). **Decision still open for Derek: leave config:printer_map vestigial (recommended — nothing regenerates it, so it's a safe read-only snapshot) vs delete.**
...
**⚠️ `position` IS the FilaBridge toolhead index (verified — Derek was right):** `position` is sent **VERBATIM** to FilaBridge as `toolhead_id`. `logic.py:140` `_fb_write` POSTs `{"printer_name":…, "toolhead_id": <position>, "spool_id":…}` to `{fb_url}/map_toolhead`; `perform_smart_move` builds `dest_th = (p['printer_name'], p['position'])` (logic.py:591) and passes `position` straight through (test pins: `position 3 → toolhead_id 3`). **The XL-5→position-5 gap (no 4) is a real hardware slot index, NOT a 0..N-1 sequence.** ⇒ **Step 4's PUT write-path MUST preserve `position` verbatim, gaps included — NEVER renumber/compact/normalize**, or FilaBridge map/unmap silently target the wrong physical toolhead. (Add a Step-4 test pinning gap preservation through a PUT.) `position` is also load-bearing for the MMU M0/M1 de-dup (two aliases share a position) + dryer-box slot ordering.
