...
| **NEW** Polling architecture reduction (L206) | Follow-up from the L28 fix. Replace per-heartbeat fan-out (~12-15 reqs/5s) with bulk `/api/dashboard_pulse` endpoint, plus visibility/idle-aware cadence. Est. ~4 hrs total. See buglist entry for full design. |
