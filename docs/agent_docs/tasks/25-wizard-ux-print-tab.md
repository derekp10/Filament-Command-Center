...
> **Status: TODO** — filed 2026-06-15 by `/refresh-groups` (25.H/25.I, grounded by a 7-agent triage workflow); **25.3 added 2026-06-29** from the Group 23 cleanup verification. Three small add/edit-wizard interaction-polish items that share one file. **Do NOT nest under the DONE Group 10 (Wizard UX Overhaul) or Group 17 (post-create chips)** — completed epics get lifted on archive, so nested follow-ups vanish ([[feedback_standalone_followups_not_under_completed_epics]]).
