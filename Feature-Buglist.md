...
* If a spool isn't activly deployed to a toolhead during an update to the Filament Command Center, it looses it's current slot assignment, and has to be reassigned. (This might happen during other senerios, but this one is the first I've noticed.) _[CONFIRMED 2026-04-26 — Reproducible. Trigger: deploying a new version/code update to production. A plain Docker container restart does NOT appear to cause it (not observed on dev). This suggests the culprit is something in the deployment or migration path that runs only when new code is loaded, not generic startup/init. Hard to test because reproduction requires a real version update cycle. Suspected cause: `container_slot` is getting cleared when `update_spool` runs without merging prior extras. This class of bug has been fixed in `perform_smart_move` (the read-merge-write pattern on line 321-330 of logic.py); there may still be surfaces that write a bare `{'extra': {'container_slot': ''}}` and clobber siblings. Next step: audit all `update_spool` callers, and investigate deployment-time code paths (migrations, setup_fields.py, any one-shot init that touches spool extras) that may re-write spool data without preserving existing `extra` fields.]_
